      ...
      <div className="flex items-center gap-4">
        {selectedSession && (
          <button
            ...
            className="p-2 rounded-lg hover:bg-white/5 text-surface-200/60 hover:text-white transition-all"
          >
      ...
      {!selectedSession ? (
        ...
        sessions.length === 0 ? (
          <div className="glass rounded-2xl p-12 text-center">
            <MessageSquare className="w-12 h-12 text-surface-200/20 mx-auto mb-4" />
            <p className="text-surface-200/50 font-medium mb-2">No conversations yet</p>
          ...
          <div className="space-y-3">
            {sessions.map((session) => (
              <button
                ...
                className="w-full glass-strong rounded-xl p-5 flex items-center gap-4 hover:bg-white/[0.06] transition-all group text-left"
              >
                <div className="w-11 h-11 rounded-xl bg-brand-600/20 flex items-center justify-center shrink-0">
                ...
                <ChevronRight className="w-5 h-5 text-surface-200/20 group-hover:text-white/40 transition-colors" />
              </button>
            ))}
          </div>
        )
        ...
        loadingMessages ? (
          ...
          <div className="space-y-4 max-w-3xl">
            {messages.map((msg) => (
              ...
              >
                {msg.role === 'assistant' && (
                  <div className="w-9 h-9 rounded-xl bg-brand-600/20 flex items-center justify-center shrink-0 mt-1">
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-5 py-3 text-sm leading-relaxed ${
                    msg.role === 'user'
                      ...
                      : 'glass-strong text-surface-200/80 rounded-bl-md'
                  }`}
                >
                  <p className="whitespace-pre-wrap">{msg.content}</p>
                  <p className={`text-[10px] mt-2 ${msg.role === 'user' ? 'text-brand-200/50' : 'text-surface-200/30'}`}>
                    {new Date(msg.created_at).toLocaleTimeString()}
                  </p>
                </div>
                {msg.role === 'user' && (
                  <div className="w-9 h-9 rounded-xl bg-surface-200/10 flex items-center justify-center shrink-0 mt-1">
